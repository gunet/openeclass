                                                ...
                                                @if (count($auth) > 1 and $alt_auth_stud_reg != FALSE) <!-- allow user registration via alt auth methods -->
                                                    ...
                                                    @foreach ($auth as $k => $v)
                                                        @if ($v != 1)  <!--  bypass the eclass auth method -->
                                                            ...
                                                            @if ($v < 8) 
                                                                <br><a class="text-primary fs-5"  href='altnewuser.php?auth={{ $v }}'>{{ get_auth_info($v) }}</a>
                                                            @else
                                                                @if($eclass_stud_reg == 1) 
                                                                    <br><a class="text-primary fs-5" href='formuser.php?auth={{ $v }}'>{{ get_auth_info($v) }}</a>
                                                                @else
                                                                    <br><a class="text-primary fs-5" href='newuser.php?auth={{ $v }}'>{{ get_auth_info($v) }}</a>
                                                                @endif
                                                            @endif
                                                        @endif
                                                    @endforeach
                                                        ...
                                                        @foreach ($auth as $k => $v)
                                                            @if ($v != 1)   <!-- bypass the eclass auth method -->
                                                                ...
                                                                @if ($v < 8) 
                                                                    ...
                                                                    <br><a class="text-primary fs-5"  href='altnewuser.php?auth={{ $v }}&p=1'>{{ get_auth_info($v) }}</a>
                                                                    @else 
                                                                    <br><a class="text-primary fs-5"  href='altnewuser.php?auth={{ $v }}'>{{ get_auth_info($v) }}</a>
                                                                    @endif
                                                                @else 
                                                                    @if ($alt_auth_prof_reg) 
                                                                        <br><a class="text-primary fs-5"  href='formuser.php?auth={{ $v }}&p=1'>{{ get_auth_info($v) }}</a>
                                                                    @else 
                                                                        <br><a class="text-primary fs-5"  href='newuser.php?auth={{ $v }}&p=1'>{{ get_auth_info($v) }}</a>
                                                                    @endif    
                                                                @endif
                                                            @endif
                                                        @endforeach
