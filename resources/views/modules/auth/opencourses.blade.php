                                ...
                                @foreach($courses as $mycourse)
                                    <tr>
                                        <td>
                                            ...
                                            @endif
                                            <div>
                                                <small class='vsmall-text Neutral-900-cl TextRegular'>{{ $mycourse->t }}</small>
                                            </div>
                                        </td>

                                        <td class='border-top-0 border-start-0 border-end-0 text-center align-middle'>
                                            @if (!get_config('show_modal_openCourses'))
                                                <a href='{{ $urlAppend }}modules/auth/info_course.php?c={{ $mycourse->k }}' class='float-start mb-2' data-bs-toggle='tooltip' data-bs-placement='top' title="{{trans('langPreview')}}" aria-label="{{ trans('langPreview') }}">
                                                    <i class="fa-solid fa-display"></i>
                                                </a>
                                            @else
                                                <button class="ClickCourse border-0 rounded-pill bg-transparent float-start mb-2" id="{{$mycourse->k}}" type="button" class="btn btn-secondary" data-bs-toggle="tooltip" data-bs-placement="top" title="{{trans('langPreview')}}" aria-label="{{trans('langPreview')}}">
                                                    <i class='fa-solid fa-display'></i>
                                                </button>
                                            ...
                                            @if ($isInOpenCoursesMode)
                                                {!! CourseXMLElement::getLevel($mycourse->level) !!}&nbsp;
                                                    <a href='javascript:showMetadata("{!! $mycourse->k !!}");'><img src='{{ $urlAppend }}resources/icons/lom.png'/></a>
                                            @else
                                                {!! course_access_icon($mycourse->visible) !!}
                                            @endif
                                        </td>
