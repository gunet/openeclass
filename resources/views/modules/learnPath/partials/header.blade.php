<nav class="navbar navbar-eclass navbar-learningPath py-0 w-100 h-100" id="lp-header" data-close-url="{{ $returl }}" data-prev-module="{{ $prevModuleAttr }}" data-next-module="{{ $nextModuleAttr }}" data-progress="{{ $progressAttr }}">
    <div class="col-12 h-100 d-flex justify-content-between align-items-center px-3 gap-3">
        ...
        <div id="navigation-btns" class="d-flex justify-content-end align-items-center gap-2">
            @if ($moduleNb > 1)
                ...
                @endif
            @endif
            <a id="lp-exit" class="btn btn-danger text-decoration-none" href="{{ $returl }}" style="min-height: 30px !important; max-height: 35px !important;">
