            ...
            @if($image_footer)
                ...
                <div class='col-12 d-flex d-flex justify-content-center align-items-center gap-3 flex-wrap mt-3'>
                    @if (!get_config('dont_display_about_menu'))
                        <div>
                            <a class="a_tools_site_footer" href="{{$urlAppend}}info/about.php" @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>
                    ...
                    @if (!get_config('dont_display_contact_menu'))
                        <div>
                            <a class="a_tools_site_footer" href="{{$urlAppend}}info/contact.php" @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>
                                {{ trans('langContact') }}
                            </a>
                        </div>
                    @endif
                    ...
                    @if (!get_config('dont_display_manual_menu'))
                        <div>
                            <a class="a_tools_site_footer" href="{{$urlAppend}}info/manual.php" @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>
                                {{ trans('langManuals') }}
                            </a>
                        </div>
                    @endif
                    <div>
                        <a class="a_tools_site_footer" href="{{$urlAppend}}info/terms.php" @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>
                            {{ trans('langUsageTerms') }}
                        </a>
                    </div>
                    @if (get_config('activate_privacy_policy_text'))
                        <div>
                            <a class="a_tools_site_footer" href="{{$urlAppend}}info/privacy_policy.php" @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>
                ...
                <div class="col-12 d-flex justify-content-center align-items-center flex-wrap gap-5 mt-3 pb-3">
                    <a class="copyright" href='{{$urlAppend}}info/copyright.php' @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>Copyright © {{ date('Y') }} All rights reserved</a>
            ...
            @else
                ...
                <nav class='col-12 d-flex justify-content-between align-items-center'>
                    <ul class="container-items-footer nav">
                        @if (!get_config('dont_display_about_menu'))
                            <li class="nav-item"><a class="nav-link menu-item a_tools_site_footer ps-2 pe-3" href="{{ $urlAppend }}info/about.php"  @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>{{ trans('langPlatformIdentity') }}</a></li>
                        @endif
                        @if (!get_config('dont_display_contact_menu'))
                            <li class="nav-item"><a class="nav-link menu-item a_tools_site_footer px-3" href="{{ $urlAppend }}info/contact.php"  @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>{{ trans('langContact') }}</a></li>
                        @endif
                        @if (!get_config('dont_display_manual_menu'))
                            <li class="nav-item"><a class="nav-link menu-item a_tools_site_footer px-3" href="{{ $urlAppend }}info/manual.php"  @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>{{ trans('langManuals') }}</a></li>
                        @endif
                        <li class="nav-item"><a class="nav-link menu-item a_tools_site_footer px-3" href="{{ $urlAppend }}info/terms.php"  @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>{{ trans('langUsageTerms') }}</a></li>
                        @if (get_config('activate_privacy_policy_text'))
                            <li class="nav-item"><a class="nav-link menu-item a_tools_site_footer px-3" href="{{ $urlAppend }}info/privacy_policy.php"  @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>{{ trans('langPrivacyPolicy') }}</a>
                        @endif
                    </ul>
                    <div class='d-flex justify-content-start align-items-center'>
                        <a class="copyright px-2" href='{{ $urlAppend }}info/copyright.php' @if(isset($_SESSION['provider']) && $_SESSION['provider'] !== 'lti_publish') target="_blank" @endif>Copyright © {{ date('Y') }} All rights reserved</a>
                        @if(get_config('enable_social_sharing_links'))
                            @if (get_config('link_fb'))
                                <a class='a_tools_site_footer' href="{!! get_config('link_fb') !!}" target="_blank" aria-label="Facebook: {{ trans('langOpenNewTab') }}">
                                    <i class="fab fa-facebook-f social-icon-tool"></i>
