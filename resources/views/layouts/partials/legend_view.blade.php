...
@if (!isset($_GET['fromFlipped']) && !isset($_SESSION['mobile']))
        ...
        <div class='d-flex flex-column'>
            ...
            @endif
                    ...
                    @if($module_id != MODULE_ID_COURSEINFO and $module_id != MODULE_ID_USERS
                        ...
                        <form id="form_id"
                              action="{{$urlAppend}}main/module_toggle.php?course={{$course_code}}&module_id={{$module_id}}"
                              method="post">
                            ...
                            @if (display_activation_link($module_id))
                                @if($visible_module == 0)
                                    <a class='btn deleteAdminBtn text-decoration-none'
                                       href="javascript:$('#form_id').submit();"
                                       data-bs-toggle="tooltip" data-bs-placement="bottom"
                                       data-bs-original-title="{{ trans('langActivate') }}"
                                       aria-label="{{ trans('langActivate') }}">
                                        <i class="fa-regular fa-eye-slash"></i>
                                    </a>
                                @else
                                    <a class='btn successAdminBtn text-decoration-none'
                                       href="javascript:$('#form_id').submit();"
                                       data-bs-toggle="tooltip" data-bs-placement="bottom"
                                       data-bs-original-title="{{ trans('langDeactivate') }}"
                                       aria-label="{{ trans('langDeactivate') }}">
                    ...
                    @if (defined('RSS'))
                        ...
                        <a class='btn btn-default text-decoration-none tiny-icon-rss' href="{{RSS}}"
                           data-bs-toggle="tooltip" data-bs-placement="bottom"
                           title="{{ defined('RSS_TITLE')? RSS_TITLE: trans('langRSSFeed') }}"
