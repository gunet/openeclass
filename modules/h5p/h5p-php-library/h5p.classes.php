...
class H5PValidator {
  ...
  public function isValidPackage($skipContent = FALSE, $upgradeOnly = FALSE) {
    ...
    global $tmpZipPath; // declaring my variable
...
class H5PStorage {
  ...
  public function savePackage($content = NULL, $contentMainId = NULL, $skipContent = FALSE, $options = array()) {
    global $tmpZipPath; // declaring custom variable
    if ($this->h5pC->mayUpdateLibraries()) {
    ...
    if (!$skipContent) {
      ...
      // starting custom gunet code. This is done to save the .h5p file in a separate folder and not delete it
      global $webDir;
      global $course_code;
      ...
      if(copy($tmpZipPath, $destextract)){
        unlink($tmpZipPath); // finally deleting the zip file
      }else{
        echo "didn't manage to move the h5p zip";
      }
      $files = "h5p.json";
      $zip = new ZipArchive;
      $res = $zip->open($destextract);
      if ($res === TRUE) {
        $zip->extractTo($dest, $files);
      $zip->close();
      }
      $this->h5pF->saveContentDependencies($content['id']); // mine custom function to insert into the db the content dependencies
  ...
  private function saveLibraries() {
    ...
    foreach ($this->h5pC->librariesJsonData as &$library) {
      ...
      $library['libraryId'] = $this->h5pF->getLibraryId($library['machineName'], $library['majorVersion'], $library['minorVersion']); // dikia mou allagh, alliws den ginotan save ta dependencies
