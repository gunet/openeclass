...
if(isset($submit)) {
	...
	if (isset($delusers)) 	
		$tool_content .= delete_users();
	if (isset($delannounces))
		$tool_content .= delete_announcements();

	mysql_select_db($currentCourseID);
        if (isset($delagenda))
		$tool_content .= delete_agenda();
	if (isset($hideworks))
		$tool_content .= hide_work();
	$tool_content .=  "<p><center><a href='infocours.php'>$langBack</a></center></p>";
				
} else {
	$tool_content .= "<blockquote>$langRefresh</blockquote>";
	$tool_content .= "<form action='refresh_course.php' method='post'>
		<table width=\"99%\"><caption>Επιλογή Ενεργειών</caption<tbody>
