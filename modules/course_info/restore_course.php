...
if (isset($_FILES['archiveZipped']) and $_FILES['archiveZipped']['size'] > 0) {
    ...
    if (file_exists($user_file)) {
        ...
        $userid_map = restore_users($course_id, unserialize(file_get_contents($user_file)), $cours_user);
    ...
    $link_map = restore_table($restoreThis, 'link', array('set' => array('course_id' => $course_id),
        ...
        'return_mapping' => 'id'));
    $ebook_map = restore_table($restoreThis, 'ebook', array('set' => array('course_id' => $course_id),
        'return_mapping' => 'id'));
    ...
    if (file_exists("$restoreThis/video"))
        $video_map = restore_table($restoreThis, 'video', array('set' => array('course_id' => $course_id),
            'return_mapping' => 'id'));
    if (file_exists("$restoreThis/videolink"))
        $videolink_map = restore_table($restoreThis, 'videolink', array('set' => array('course_id' => $course_id),
            'return_mapping' => 'id'));
    if (file_exists("$restoreThis/dropbox_file") &&
        ...
        $dropbox_map = restore_table($restoreThis, 'dropbox_file', array('set' => array('course_id' => $course_id),
            ...
            'return_mapping' => 'id'));
        restore_table($restoreThis, 'dropbox_person', array('map' => array('file_id' => $dropbox_map,
                'person_id' => $userid_map)));
        restore_table($restoreThis, 'dropbox_post', array('map' => array('file_id' => $dropbox_map,
    ...
    if (file_exists("$restoreThis/agenda")) {
        restore_table($restoreThis, 'agenda', array('delete' => array('id'),
    ...
    if (file_exists("$restoreThis/exercise") &&
        ...
        $exercise_map = restore_table($restoreThis, 'exercise', array('set' => array('course_id' => $course_id),
            'return_mapping' => 'id'));
    ...
    $unit_map = restore_table($restoreThis, 'course_units', array('set' => array('course_id' => $course_id),
        'return_mapping' => 'id'));
...
function user($userid, $givenname, $surname, $login, $password, $email, $status, $phone, $department, $registered_at = NULL, $expires_at = NULL, $inst_id = NULL) {
    ...
    if (mysql_num_rows($u) > 0) {
        ...
        echo sprintf($langRestoreUserExists, '<b>' . q($login) . '</b>', '<i>' . q("$res[givenname] $res[surname]") . '</i>', '<i>' . q("$name $surname") . '</i>'), '<br>';
        ...
        echo sprintf($langRestoreUserNew, '<b>' . q($login), '</b>', '<i>' . q("$name $surname") . '</i>'), '<br>';
    } else {
        return;
    }
    ...
    foreach ($backup as $data) {
        ...
        if ($do_insert) {
            if (isset($url_prefix_map)) {
                db_query(strtr($sql_intro . field_values($data, $set), $url_prefix_map));
            } else {
