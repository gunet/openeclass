...
for($i = 0; $i < $total_categories; $i++) {
   if($viewcat != -1) {
	...
	<FONT FACE=\"$FontFace\" SIZE=\"$FontSize2\" COLOR=\"$textcolor\"><B>

	<a href=\"$PHP_SELF?viewcat=".$categories[$i]["cat_id"]."\">$title</a></B></FONT></TD></TR>";
	continue;
     }
   }
   $title = stripslashes($categories[$i][cat_title]);
   ...
   for($x = 0; $x < count($forum_row); $x++)
	 ...
	 if($last_post_time > $last_visit && $last_post != "No posts") {
		...
		<a href=\"viewforum.$phpEx?forum=".$forum_row[$x]["forum_id"]."&$total_posts\">$name</a></font>\n";
