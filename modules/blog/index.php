    ...
    if ($num_posts == 0) {//no blog posts
        ...
        foreach ($posts as $post) {
            ...
            if ($comments_enabled && ($post->getCommenting() == 1)) {
                $comm = new Commenting('blogpost', $post->getId());
                $comment_content = "<a class='btn btn-primary btn-sm float-end' href='$_SERVER[SCRIPT_NAME]?$url_params&amp;action=showPost&amp;pId=".$post->getId()."#comments_title'>$langComments (".$comm->getCommentsNum().")</a>";
            } else {
