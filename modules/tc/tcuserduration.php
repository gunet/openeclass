...
if (isset($_GET['per_user']) or isset($_GET['u'])) { // all-users participation in meetings
    ...
    if (isset($_GET['u']) and $_GET['u']) { // participation for a specific user
        ...
        $tool_content .= "<div class='table-responsive'><table class='table-default table-tc-duration'>";
        $tool_content .= "
                    <thead><tr>
...
if (isset($_GET['pdf']) and $is_course_reviewer) {
    $pdf_title = "$course_code tc_report";
    $course_title = q("$currentCourseName - $langStatsReport");
    $module_type_title = q($langStatsReport);
