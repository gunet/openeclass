    ...
    public function put($courseCode, $isEditor, $uid, $always_open = false) {
        ...
        if (is_null($courseCode)) { //for the case of personal blog posts comments
            ...
            if (Commenting::permCreate($isEditor, $uid, course_code_to_id($courseCode))) {
                $out .= '<div class="col-12"><div class="form-wrapper form-edit p-3 rounded"><form action="" onsubmit="xmlhttpPost(\''.$urlServer.'modules/comments/comments.php?course='.$courseCode.'\', \'new\','.$this->rid.', \''.$this->rtype.'\', \''.$langCommentsSaveConfirm.'\'); return false;">';
                $out .= '<textarea class="form-control" name="textarea" id="textarea-'.$this->rid.'" rows="5"></textarea><br/>';
                $out .= '<input class="btn btn-sm btn-primary submitAdminBtn w-50 m-auto d-block" name="send_button" type="submit" value="'.$langSubmit.'" />';
                $out .= '</form></div></div>';
            }
