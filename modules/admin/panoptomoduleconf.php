...
if (isset($_GET['add_template'])) {
    ...
    $pageName = $langNewPanoptoTool;
    $navigation[] = array('url' => 'panoptomoduleconf.php', 'name' => $langPanoptoConf);
    ...
    Session::flash('message', $langPanoptoAppDeleteSuccessful);
    Session::flash('alert-class', 'alert-success');
    redirect_to_home_page("modules/admin/panoptomoduleconf.php");
    ...
    add_update_lti_app(
        ...
        $_POST['lti_key'],
        $_POST['lti_secret'],
        $_POST['lti_public_keyset_url'],
        $_POST['lti_initiate_login_url'],
        $_POST['lti_redirection_uri'],
        $_POST['lti_launchcontainer'],
        $_POST['status'],
        $_POST['lti_courses'],
        LTI_TYPE,
        null,
    ...
    Session::flash('message', $langPanoptoAppAddSuccessful);
    Session::flash('alert-class', 'alert-success');
    redirect_to_home_page("modules/admin/panoptomoduleconf.php");
    ...
    $pageName = $langPanoptoConfDetails;
    $navigation[] = array('url' => 'panoptomoduleconf.php', 'name' => $langPanoptoConf);
    $tool_content .= action_bar(array(
        array('title' => $langBack,
            'url' => "panoptomoduleconf.php",
            'icon' => 'fa-reply',
            'level' => 'primary')));
    ...
    if (isset($_GET['edit_template'])) {
    ...
    } else { //display available templates

        $tool_content .= action_bar(array(
            array(
                'title' => $langNewPanoptoTool,
                'url' => "panoptomoduleconf.php?add_template",
                'icon' => 'fa-plus-circle',
                'level' => 'primary-label',
        ...
        $q = Database::get()->queryArray("SELECT * FROM lti_apps WHERE is_template = true AND type = 'panopto' AND course_id is null ORDER BY title ASC");
        if (count($q) > 0) {
            ...
            $tool_content .= "<div class='col-sm-12'><div class='alert alert-warning'><i class='fa-solid fa-triangle-exclamation fa-lg'></i><span>$langNoAvailablePanoptoTemplates</span></div></div>";
        }
    }
}
