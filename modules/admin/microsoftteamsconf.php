...
require_once 'modules/admin/extconfig/microsoftteamsapp.php';

$app = ExtAppManager::getApp('microsoftteams');
...
if (isset($_POST['submit'])) {
    ...
    if ($_POST['submit'] == 'clear') {
        ...
        Database::get()->query("INSERT INTO tc_servers (`type`, hostname, api_url, enabled, all_courses) 
                                            VALUES('microsoftteams', ?s, ?s, ?s, ?s)
                                        ON DUPLICATE KEY UPDATE enabled = ?s, all_courses = ?s",
            $app::MICROSOFTTEAMSURL, $app::MICROSOFTTEAMSURL, $enabled, $all_courses, $enabled, $all_courses);

        $tc_id = Database::get()->querySingle("SELECT id FROM tc_servers WHERE `type` = 'microsoftteams'")->id;
        ...
        if ($result) {
            ...
            Session::flash($result, 'alert-danger');
        } else {
            Session::flash('message', $langFileUpdatedSuccess);
            Session::flash($result, 'alert-success');
        }
...
$boolean_field = "";
$tool_content .= "
    <div class='d-lg-flex gap-4 mt-4'><div class='flex-grow-1'>
    <div class='col-12'>
