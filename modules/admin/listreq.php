...
if (!empty($show) && ($show=="closed")) {
	if (!empty($id) && ($id>0)) {
		...
		$tool_content .= "\n    <td align=\"center\">
		<small>".nice_format(date("Y-m-d", strtotime($req[7])))."</small></td>";
            	$tool_content .= "\n    <td align=\"center\">
		<small>".nice_format(date("Y-m-d", strtotime($req[8])))."</small></td>";
            	$tool_content .= "\n    <td>".$req[9]."</td>";
		$tool_content .= "\n    <td align=center>
		<a href=\"listreq.php?id=$req[rid]&"."show=closed\">$langRestore</a></td>\n  </tr>";
		$k++;
		}
	}
	$tool_content .= "\n  </tbody>\n  </table>\n";
...
if (!empty($show)) {
	$tool_content .= "<p align=\"right\"><a href=\"listreq.php\">$langBackRequests</a></p><br>";
}
