...
elseif (isset($_GET['action']) && $_GET['action'] == 'add') {
    if (isset($_POST['add'])) {
        ...
        list($names, $name, $multiple, $searchable, $active) = prepareDataFromPost();
...
elseif (isset($_GET['action']) and $_GET['action'] == 'edit') {
    ...
    if (isset($_POST['edit'])) {
        ...
        if (empty($names)) {
            ...
            $q = "UPDATE category SET name = ?s, multiple = ?d, searchable = ?d, active = ?d WHERE id = ?d";
            Database::get()->query($q, $name, $multiple, $searchable, $active, $id);
            Session::Messages($langEditCourseCategorySuccess, 'alert alert-success');
            redirect_to_home_page('modules/admin/coursecategory.php');
        }
    } else {
        // Display form for edit course category information
        $mycat = Database::get()->querySingle("SELECT name, multiple, searchable, active FROM category WHERE id = ?d", $id);
        $tool_content .= displayForm($id, $mycat->name, $mycat->multiple, $mycat->searchable, $mycat->active);
    }
}
...
function prepareDataFromPost() {
    ...
    return array($names, $name, $multiple, $searchable, $active);
}

function displayForm($id = null, $name = null, $multiple = null, $searchable = null, $active = null) {
