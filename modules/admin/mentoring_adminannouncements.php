...
if (isset($_GET['vis'])) {
    $id = $_GET['id'];
    $vis = $_GET['vis'] ? 0 : 1;  
    Database::get()->query("UPDATE mentoring_admin_announcement SET visible = ?b WHERE id = ?d", $vis, $id);
    redirect_to_home_page('modules/admin/mentoring_adminannouncements.php');
} elseif (isset($_GET['delete'])) {
    // delete announcement command
    $id = intval($_GET['delete']);
    Database::get()->query("DELETE FROM mentoring_admin_announcement WHERE id = ?d", $id)->affectedRows;
    ...
    if($v->validate()) {
        ...
        if (isset($_POST['id'])) {
            // modify announcement
            $id = $_POST['id'];
            Database::get()->query("UPDATE mentoring_admin_announcement
            ...
            // order
            $orderMax = Database::get()->querySingle("SELECT MAX(`order`) as max FROM mentoring_admin_announcement")->max;
            $order = $orderMax + 1;
            Database::get()->query("INSERT INTO mentoring_admin_announcement
                            SET title = ?s,
                                body = ?s,
                                lang = ?s,
        ...
        redirect_to_home_page("modules/admin/mentoring_adminannouncements.php?addAnnounce=1");
    }
...
$no_order = Database::get()->querySingle("SELECT id, `order` FROM mentoring_admin_announcement WHERE `order`=0");
if ($no_order) {
    Database::get()->query("UPDATE mentoring_admin_announcement SET `order`=`id`+1");
}

if (isset($thisAnnouncementId) && $thisAnnouncementId && isset($sortDirection) && $sortDirection) {
    Database::get()->queryFunc("SELECT id, `order` FROM mentoring_admin_announcement ORDER BY `order` $sortDirection", function ($announcement) use(&$thisAnnouncementOrderFound, &$nextAnnouncementId, &$nextAnnouncementOrder, &$thisAnnouncementOrder, &$thisAnnouncementId) {
        if (isset($thisAnnouncementOrderFound) && $thisAnnouncementOrderFound == true) {
            ...
            Database::get()->query("UPDATE mentoring_admin_announcement SET `order` = ?s WHERE id = ?d", $nextAnnouncementOrder, $thisAnnouncementId);
            Database::get()->query("UPDATE mentoring_admin_announcement SET `order` = ?s WHERE id = ?d", $thisAnnouncementOrder, $nextAnnouncementId);
            return true;
        }
        // find the order
...
$head_content .= "<script type='text/javascript'>
        $(document).ready(function () {
            $('.table_td_body').each(function() {
                $(this).trunk8({
                    lines: '3',
                    fill: '&hellip;<div class=\"clearfix\"></div><a style=\"float:right;\" href=\"mentoring_adminannouncements_single.php?ann_id='
...
if (isset($_GET['addAnnounce']) || isset($_GET['modify'])) {
    ...
    if (isset($_GET['modify'])) {
        ...
        $data['announcement'] = $announcement = Database::get()->querySingle("SELECT id, title, body, `date`, `begin`,`end`,
                                                    lang, `order`, visible FROM mentoring_admin_announcement WHERE id = ?d", $id);         
    }
    ...
    $view = 'admin.mentoring_platform.announcements.create';
} 

elseif (isset($_GET['ann_id'])) {
    
    $row = Database::get()->querySingle("SELECT * FROM mentoring_admin_announcement WHERE id = ". intval($_GET['ann_id']));
    if(empty($row)){
        redirect_to_home_page("modules/admin/mentoring_adminannouncements.php");
    }else{
        ...
        $view = 'admin.mentoring_platform.announcements.show';
    }
}
    ...
    $data['announcements'] = Database::get()->queryArray("SELECT * FROM mentoring_admin_announcement ORDER BY `order` DESC");
    $view = 'admin.mentoring_platform.announcements.index';
