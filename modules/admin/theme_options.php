    ...
    if (get_file_extension($_FILES['themeFile']['name']) == 'zip') {
        ...
        if (move_uploaded_file($_FILES['themeFile']['tmp_name'], "courses/theme_data/$file_name")) {
            ...
            if ($archive->open("courses/theme_data/$file_name")) {
                ...
                $new_theme_id = Database::get()->query("INSERT INTO theme_options (name, styles, version, tenant_id) VALUES(?s, ?s, 4, ?d)", $theme_options->name, $theme_options->styles, $tenant ? $tenant->id : null)->lastInsertID;
