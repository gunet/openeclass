...
if (isset($_POST['dellall_submit']) or isset($_POST['activate_submit']) or isset($_POST['move_submit'])) {
    ...
    Database::get()->queryFunc($qry
            , function($users) use(&$data) {
        $data['usernames'] .= $users->username . "\n";
    }, $terms);
}
...
if (isset($_POST['activate_submit'])) {
    $data['infoText'] = $langActivateUserInfo;
    $data['monthsField'] = "
    ...
    $head_content .= $js;
    $data['infoText'] = sprintf($langMoveUserInfo, '<b>' . q($tree->getNodeName($dep)) . '</b>');
    $data['monthsField'] = "
    ...
    $data['infoText'] = $langMultiDelUserInfo;
    $data['monthsField'] = "<input type='hidden' name='delete' value='true'>";
    $data['confirm'] = " onclick='return confirmation(\"" . q($langMultiDelUserConfirm) . "\");'";
}
