...
if (isset($_GET['c'])) {
    ...
    $q2 = Database::get()->queryArray("SELECT COUNT(*) AS registrations, MONTH(reg_date) AS month, YEAR(reg_date) AS year FROM course_user
                WHERE course_id = ?d AND (reg_date BETWEEN ?s AND ?s)
                    AND status = " . USER_STUDENT . " GROUP BY month, year, reg_date ORDER BY reg_date ASC",
    ...
    foreach ($q3 as $data) {
            $csv->outputRecord($data->month . '-' . $data->year, $data->visits, $data->users);
            $total_visits += $data->visits;
            $total_users += $data->users;
    }
    $csv->outputRecord($langTotal, $total_visits, $total_users);
