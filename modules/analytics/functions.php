...
function display_learning_analytics() {
    global $course_id, $course_code, $tool_content, $langLearningAnalytic, $langAnalyticsNoAnalytics, $langActive, $langInactive,
    ...
    if(count($sql_data) == 0) {
        $results = "<p class='text-center text-muted'>$langAnalyticsNoAnalytics</p>";
    } else {
        $results = "";
    ...
    foreach ($analytics_elements as $analytics_element) {
        ...
        if (sizeof($critical) == 0){
            ...
            foreach ($critical as $crit) {
                $userid = $crit['id'];
                $bad_results .="<div class='row res-table-row'>
                    <div class='col-sm-5'>".
                    $crit['givenname'] . " " . $crit['surname']
        ...
        if (sizeof($advanced) == 0) {
            $good_results = '<div>' . $langAnalyticsNoAdvanced . '</div>';
        } else {
            ...
            <div class='col-xs-12'>
                <div class='panel panel-default'>
                    <div class='panel-body'>
                        ...
                        <div class='col-sm-12'>
                        <div class='col-sm-6'><h4 class='text-success'>$langAnalyticsAdvancedLevel</h4>
                            <div class='res-table-wrapper'>
                                $good_results
                            </div>
                        </div>
                        
                        <div class='col-sm-6'><h4 class='text-danger'>$langAnalyticsCriticalLevel</h4>
                            <div class='res-table-wrapper'>
                                $bad_results
                            </div>
                        </div>
...
function display_analytics_elements($analytics_id) {
    global $tool_content, $course_code, $langAdd, $langAnalyticsNoElements, $langAnalyticsType, $langAnalyticsGradeLimits, $langAnalyticsThresholds,
    $langAnalyticsWeight, $langAnalyticsCriticalLevel, $langAnalyticsAdvancedLevel, $langModify, $langDelete, $langAnalyticsConfirmDeletion,
            ...
            <div class='panel panel-default'>
                <div class='panel-body'>
                    ...
                    <div class='row'>
                        ...
                        <div class='col-sm-5'>
                            <div class='pn-info-title-sct'>$langAnaliticsTimeFrame</div>
                            <div class='pn-info-text-sct'>$langFrom $start_date $langTill $end_date</div>
                            <div class='pn-info-title-sct'>$langAnalyticsCalculation</div>
...
function display_analytics_peruser($analytics_id, $startdate, $enddate, $previous, $next, $orderby, $reverse, $period, $download) {
    ...
    $langAnalyticsAdvancedLevel, $langAnalyticsMiddleLevel, $langAnalyticsCriticalLevel, $langAnalyticsDetails, $langMessage, $langAnalyticsExport;
    //To change here before commit
    //$sql_data = Database::get()->queryArray("SELECT u.givenname as givenname, u.surname as surname, cu.user_id as userid FROM course_user as cu inner join user as u on cu.user_id=u.id WHERE course_id = ?d AND cu.status = 5", $course_id);
    $sql_data = Database::get()->queryArray("SELECT u.givenname as givenname, u.surname as surname, cu.user_id as userid FROM course_user as cu inner join user as u on cu.user_id=u.id WHERE course_id = ?d", $course_id);
    ...
    <div class='row'>
        ...
        <div  class='col-xs-3'>".
            action_bar(
                array(                  
                    array('title' => $langAnalyticsExport,                    
...
function display_analytics_user($userid,$analytics_id, $start, $end, $previous, $next) {
    global $tool_content, $course_id, $course_code, $langSurnameName, $langPercentage;
    $backclass = '';
    if (is_null($previous))
        $backclass = 'style="display:none"';
        ...
        <div class='col-xs-12'>
            <div class='panel panel-default'>
                <div class='panel-body'>
                    ...
                    <div class='row'>
                        <div class='col-sm-7'>
                            <div class='pn-info-title-sct'>$langEmail</div>
                            $email
                        </div>
                        <div class='col-sm-5'>
                            <div class='pn-info-title-sct'>$langAm</div>
                            $am
                            <div class='pn-info-title-sct'>$langPhone</div>
                            $phone
...
function edit_analytics_settings ($analytics_id = 0)
{
    ...
    <form class='form-horizontal' role='form' method='post' action='$_SERVER[SCRIPT_NAME]?course=$course_code'>
        ...
        <div class='form-group'>
            <label for='title' class='col-sm-2 control-label'>$langActivate</label>
                <div class='col-sm-10'>";
                    $tool_content .= selection(get_activation_status(), 'active', $active);
                $tool_content .= "</div>
        </div>
        <div class='form-group'>
        <label for='title' class='col-sm-2 control-label'>$langAnalyticsCalculation</label>
            <div class='col-sm-10'>";
                $tool_content .= selection(get_period_types_array () , 'periodType', $periodType);
            $tool_content .= "</div>
        </div>
        <div class='form-group'>
            <label class='col-sm-2 control-label'>$langStart:</label>
            <div class='col-sm-10'>
                <div class='input'>
        ...
        <div class='form-group'>
            <label class='col-sm-2 control-label'>$langFinish:</label>
            <div class='col-sm-10'>
                <div class='input'>
...
function analytics_element_form($analytics_id, $type=null, $analytics_element_id=0) {
    global $tool_content, $course_code, $langAnalyticsCriticalLevel, $langAnalyticsMinValue, $langAnalyticsWeight, $langAnalyticsMaxValue, $langAnalyticsAdvancedLevel, $langSave, $langAdd;
    ...
    $tool_content .="<h3>" . $elementTypeTitle ."</h3>
    <div class='form-wrapper'><form class='form-horizontal' role='form' method='post' action='$_SERVER[SCRIPT_NAME]?course=$course_code'>
        ...
        <div class='form-group'>
            <label for='title' class='col-sm-4 control-label' style='text-align:left'>$langAnalyticsCriticalLevel:</label>
        ...
        <div class='form-group'>
            <label for='title' class='col-sm-4 control-label' style='text-align:left'>$langAnalyticsAdvancedLevel:</label>
...
function get_available_resources($type, $analytics_element_id) {
    global $course_id, $analytics_id, $langAnalyticsResourceNotAvailable;
    ...
    $resource_field =  "<div class='form-group'>
                            <label for='title' class='col-sm-2 control-label'>Resource</label>
                            <div class='col-sm-10'>"
