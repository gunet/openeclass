            ...
            $tool_content .= "
                ...
                foreach($mtitles_in_class as $title_class) {
                    ...
                    if($q) {
                        ...
                        $tool_content .= "
                            <td><input type='checkbox' name='in_class[]' id='".$unit_id."_". $act_id."' value='".$unit_id."_".$act_id."'></input></td>
                        ";
                    }
    ...
    if(!isset($_GET['edit_act'])){ //complete actions if it is creation
        $language = $_SESSION['language'];
            ...
            Database::get()->query("INSERT INTO course_description SET
                        ...
                        `order` = ?d,
                        `visible` =?d,
                        update_dt = NOW()", $new_course_id, $langILOtype, purify($commentsGoals), 2, $maxOrderGoal->morder, 1
            );
