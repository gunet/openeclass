...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $all_announc = Database::get()->querySingle("SELECT COUNT(*) AS total FROM mentoring_admin_announcement");
    $filtered_announc = Database::get()->querySingle("SELECT COUNT(*) AS total FROM mentoring_admin_announcement WHERE title LIKE ?s", $keyword);
    if ($limit>0) {
        $extra_sql = 'LIMIT ?d, ?d';
    ...
    $result = Database::get()->queryArray("SELECT * FROM mentoring_admin_announcement WHERE title LIKE ?s $student_sql ORDER BY `order` DESC , `date` DESC  $extra_sql", $keyword, $extra_terms);
...
if (isset($_GET['an_id'])) {
    ...
    $row = Database::get()->querySingle("SELECT * FROM mentoring_admin_announcement WHERE id = ". intval($_GET['an_id']));
    if(empty($row)){
        redirect_to_home_page("modules/mentoring/announcements/system_announcements.php");
    }
    ...
    view('info.mentoring_single_system_announcement', $data);
} else {
    ...
    $head_content .= "<script type='text/javascript'>
        $(document).ready(function() {
           ...
           var oTable = $('#mentoring_ann_table_admin_logout').DataTable ({
                'bStateSave': true,
                'bProcessing': true,
                'bServerSide': true,
                'sScrollX': true,
                ...
                'fnDrawCallback': function( oSettings ) {
                    ...
                    $('#mentoring_ann_table_admin_logout_filter label input').attr({
    ...
    view('info.mentoring_system_announcements', $data);
}
