...
function list_docs($id = NULL, $subsystem = NULL, $expanded = false) {
    global $mentoring_program_code, $langNoDocuments, $urlServer, $program_group_id;
    
    load_js('jstree3');
    
    if (is_null($subsystem)) { //group documents
        $div_id = 'jstree_commondoc';
        $sys = '&group_id='.$program_group_id;
        $input_id = 'docs';
                   ...
                   $(function () {
                     $('#$div_id').jstree({
                       ...
                       'core': {
	                     ...
	                     'data' : {
                           'url' : '".$urlServer."modules/mentoring/programs/wall/load_doc.php?program=$mentoring_program_code$sys$id_str',
                           'data' : function (node) {
                             return { 'id' : node.id };
                           }
                         }
            ...
            $ret_str .= "
                         $('#$div_id').on('ready.jstree', function(e, data) {
                             if(data.instance._cnt == 0) {
                                 $('#".$input_id."_div').html('<div class=\"alert alert-warning rounded-2\">$langNoDocuments</div>');
                             } else {
