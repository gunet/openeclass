...
class Question {
    ...
    /**
     * @brief Get the certainy results for a given question
     * @param $correct
     * @param $wrong
     * @return array
     */
    public function certaintyBasedResults($eid, $correct = false, $wrong = false) {

        global $exerciseCalcGradeMethod;

        $correct_answers_certainty = $wrong_answers_certainty = [];

        if ($exerciseCalcGradeMethod == CALC_GRADE_METHOD_CERTAINTY_BASED
            && ($this->type == UNIQUE_ANSWER || $this->type == TRUE_FALSE)) {
            if ($correct) { // correct answers with certainty
                $q1 = Database::get()->queryArray("SELECT COUNT(*) AS cnt, certainty AS certainty 
                                            FROM exercise_answer_record JOIN exercise_user_record 
                                                ON exercise_user_record.eurid = exercise_answer_record.eurid 
                                           WHERE question_id = ?d
                                           AND eid = ?d 
                                           AND weight > 0 
                                           GROUP BY certainty 
                ...
                if (count($q1) > 0) {
                    foreach ($q1 as $row) {
                        $correct_answers_certainty[] = ['count' => $row->cnt, 'certainty' => $row->certainty];
                    }
                }
                return $correct_answers_certainty;
            }

            if ($wrong) { // wrong answers with certainty
                $q2 = Database::get()->queryArray("SELECT COUNT(*) AS cnt, certainty AS certainty 
                                            FROM exercise_answer_record JOIN exercise_user_record 
                                                ON exercise_user_record.eurid = exercise_answer_record.eurid                                           
                                            WHERE question_id = ?d
                                            AND eid = ?d 
                ...
                if (count($q2) > 0) {
                    foreach ($q2 as $row) {
                        $wrong_answers_certainty[] = ['count' => $row->cnt, 'certainty' => $row->certainty];
                    }
                }
            }
            return $wrong_answers_certainty;
        }
    ...
    /**
     * @brief Get legend for certainty-based grade calculation
     * @param $certainty
     * @return string|void
     */
        public function getCertaintyLegend($certainty) {

            global $langNotKnow, $langNotSure, $langSure;

            switch ($certainty) {
                case 1: return $langNotKnow;
                case 2: return $langNotSure;
                case 3: return $langSure;
            }
        }

        public function getCertaintyLegend2($cw, $certainty)
        {
            global $langMastery, $langMasteryTooltip,
                   $langUnderconfident, $langUnderconfidentTooltip,
                   $langGuessingRight, $langGuessingRightTooltip,
                   $langMisconception, $langMisconceptionTooltip,
                   $langGuessingWrong, $langGuessingWrongTooltip,
                   $langKnownGap, $langKnownGapTooltip;

            if ($cw == 1) {
                switch ($certainty) {
                    case 1: return ['text' => $langMastery, 'tooltip' => $langMasteryTooltip];
                    case 2: return ['text' => $langUnderconfident, 'tooltip' => $langUnderconfidentTooltip];
                    case 3: return ['text' => $langGuessingRight, 'tooltip' => $langGuessingRightTooltip];
                }
            } else {
                switch ($certainty) {
                    case 1: return ['text' => $langMisconception, 'tooltip' => $langMisconceptionTooltip];
                    case 2: return ['text' => $langGuessingWrong, 'tooltip' => $langGuessingWrongTooltip];
                    case 3: return ['text' => $langKnownGap, 'tooltip' => $langKnownGapTooltip];
                }
            }
