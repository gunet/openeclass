...
class FreeTextAnswer extends QuestionType
{
    ...
    public function QuestionResult($choice, $eurid, $regrade, $extra_type = ''): string
    {
        ...
        global $questionScore, $question_weight,
               ...
               $langEvaluatingResponseWithAI, $course_id;
        ...
        $ai_evaluation = Database::get()->querySingle("SELECT * FROM exercise_ai_evaluation WHERE student_record_id = ?d", $eurid);
        if ($ai_evaluation) {
            ...
            if ($aiService->isEnabledForCourse(AI_MODULE_FREE_TEXT_EVALUATION) && $aiService->isEnabledForQuestion($questionId, $course_id)) {
                ...
                $html_content .= "<tr><td>";
                //$html_content .= "<div class='mt-3 p-3 bg-light border-start border-info border-4' id='ai-eval-container-{$row->answer_record_id}'>";
                $html_content .= "<h6 class='text-info'><i class='fa fa-robot'></i> $langAIEvaluation</h6>";
                //$html_content .= "<div id='ai-eval-status-{$row->answer_record_id}' class='ai-eval-pending' data-answer-id='{$row->answer_record_id}'>";
                ...
                $html_content .= "</div>";
                $html_content .= "</div>";
                $html_content .= "<div id='ai-eval-result-{$row->answer_record_id}' style='display: none;'></div>";
                $html_content .= "</div>";
                $html_content .= "</td></tr>";
            }
        }
