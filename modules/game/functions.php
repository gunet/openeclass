        ...
        foreach ($result as $details) {            
            ...
            $tool_content .= "<tr><td>";
             if (empty($details->resource)) {
                $tool_content .= "<a href='$_SERVER[SCRIPT_NAME]?course=$course_code&amp;certificate_id=$certificate_id&amp;ins=" . getIndirectReference($details->id). "'>Οποιαδήποτε</a>";
            } else if (empty($details->title)) {
                $tool_content .= "<a href='$_SERVER[SCRIPT_NAME]?course=$course_code&amp;certificate_id=$certificate_id&amp;ins=" . getIndirectReference($details->id). "'>$langGradebookNoTitle</a>";
            } else {
                $tool_content .= "<a href='$_SERVER[SCRIPT_NAME]?course=$course_code&amp;certificate_id=$certificate_id&amp;ins=" . getIndirectReference($details->id) . "'>".q($details->title)."</a>";
            }
            $tool_content .= "</td><td>";
...
function certificate_display_available_exercises($certificate_id) {
    ...
    $checkForExer = Database::get()->queryArray("SELECT * FROM exercise WHERE exercise.course_id = ?d 
                                ...
                                NOT IN (SELECT resource FROM certificate_criterion WHERE certificate = ?d AND resource!='' )", $course_id, $certificate_id);
    ...
    if ($checkForExerNumber > 0) {
        ...
        //if(!isset($check) && $check ==""){
            $tool_content .= "<table class='table-default'>";
            $tool_content .= "<tr class='list-header'><th>Οποιαδήποτε άσκηση</th>";
       ...
       if($id!=""){ 
            ...
            if ($checkForExe) {
                ...
                Database::get()->query("INSERT INTO certificate_criterion 
                                        ...
                                        $certificate_id, $module_auto_id, $module_auto_type);            
                /*
                $sql = Database::get()->queryArray("SELECT uid FROM attendance_users WHERE attendance_id = ?d", $attendance_id);
