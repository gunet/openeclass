...
function createguest($username, $course_id, $password) {
    ...
    $q = db_query("SELECT user_id from course_user WHERE status=" . USER_GUEST . " AND course_id = $course_id");
       if (mysql_num_rows($q) > 0) {       
        ...
        db_query("UPDATE user SET password = '$password' WHERE id = $guest_id");
    } else {
        db_query("INSERT INTO user (surname, givenname, username, password, status, registered_at, expires_at, whitelist, description)
                  VALUES ('$langGuestSurname', '$langGuestName', '$username', '$password', " . USER_GUEST . ", ".DBHelper::timeAfter().", ".DBHelper::timeAfter(get_config('account_duration')).", '','')");
