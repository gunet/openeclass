...
$w = new wiki();
$w->load($wikiId);
if (!$is_editor and !$w->getVisibility()) {
    header("Location: index.php?course=$course_code");
    exit();
}
...
switch ($action) {
    ...
    case "recent": {
            if (is_array($recentChanges)) {
                ...
                foreach ($recentChanges as $recentChange) {
                    ...
                    $entry = '<a class="list-group-item bg-transparent" href="' . $_SERVER['SCRIPT_NAME'] . '?course=' . $course_code . '&amp;wikiId='
    ...
    case "all": {
            ...
            $tool_content .= '<div class="col-sm-12"><div class="list-group"><a class="list-group-item bg-transparent" href="' . $_SERVER['SCRIPT_NAME'] . '?course=' . $course_code
                    . '&amp;wikiId=' . $wikiId
            ...
            if (is_array($allPages)) {
                foreach ($allPages as $page) {
                    ...
                    $link = '<a class="list-group-item bg-transparent" href="' . $_SERVER['SCRIPT_NAME'] . '?course=' . $course_code . '&amp;wikiId='
    ...
    case "history": {
        ...
        $tool_content .= '</div>';
        $tool_content .= '<form id="differences" method="GET" action="' . $_SERVER['SCRIPT_NAME'] . '">';
    ...
    case 'exSearch':
    {
        $tool_content .= '<h5>' . $langWikiSearchResults . '</h5>';
