...
class IMSCPExport
{
     ...
     * @author Amand Tihon <amand@alrj.org>
     */
    function IMSCPExport($learnPathId, $language)
    {
        /* Default values */
...
';
    ...
    function prepare()
    {
        ...
        if ( !claro_copy_file('export/APIWrapper.js', $this->destDir)
          || !claro_copy_file('export/scores.js', $this->destDir)
          || !claro_copy_file('export/imscp_v1p2.xsd', $this->destDir) )
        {
    ...
    function createManifest()
    {
         ...
         * @author Thanos Kyritsis <atkyritsis@upnet.gr>
         */
        function makeMetaData($title, $description, $language)
        {
...
<frameset border="0" rows="100%" onload="immediateComplete()">
    <frame src="' . $targetPath . '" scrolling="auto">
        ...
        // Prepare Metadata
        $metadata = makeMetaData($this->name, $this->comment, $this->language);
...
endif; // !class_exists(IMSCPExport)
