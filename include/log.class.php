...
class Log
{
    ...
    public function display($course_id, $user_id, $module_id, $logtype, $date_from, $date_now)
    {
        ...
        if ($num_of_logs > 0) {
            ...
            foreach ($sql as $r) {
                ...
                if (isset($_SESSION['is_departmentmanage_user']) && $_SESSION['is_departmentmanage_user'] && $r->action_type == LOG_DELETE_USER) {
                    $details = @unserialize($r->details);
                    $tenant_id = $details['tenant'] ?? null;

                    if ($tenant_id != getCurrentTenant()->id) {
                        continue;
                    }
                }
     ...
     * Note: $module_id = $course_id = 0 means other logging (e.g. modify user profile, course creation etc.)
     * @param int $course_id (-1 means all courses)
     * @param array $user_ids (get multiple users as array)
     * @param int $module_id (-1 means all modules)
     * @param type $logtype (-1 means logtypes)
     * @param type $date_from
     * @param type $date_now
    ...
    public function displayMultiple($course_id, array $user_ids, $module_id, $logtype, $date_from, $date_now)
    {
        global $tool_content, $modules, $langCourseTools,
            $langNoUsersLog, $langDate, $langUser, $langAction, $langDetail, $langConfig,
            $langCourse, $langModule, $langAdminUsers, $langExternalLinks, $langCourseInfo,
        ...
        $q2 = $q3 = $q4 = '';
        $q2_terms = $q3_terms = $q4_terms = [];

        if ($logtype > 0) {
            $q3 = "AND action_type = ?d";
            $q3_terms = $logtype;
            if ($logtype > 3) {
                $module_id = $course_id = 0;
            }
        ...
        // count logs
        $num_of_logs = Database::get()->querySingle("
            SELECT COUNT(*) AS count FROM log 
            WHERE user_id IN ($user_ids_str) 
            AND ts BETWEEN ?t AND ?t $q2 $q3 $q4
        ", $date_from, $date_now, $q2_terms, $q3_terms, $q4_terms)->count;

        // fetch logs
        $sql = Database::get()->queryArray("
            SELECT l.*, u.givenname, u.surname
            FROM log AS l
            JOIN user AS u ON u.id = l.user_id
            WHERE l.user_id IN ($user_ids_str)
            AND l.ts BETWEEN ?t AND ?t
            $q2 $q3 $q4
            ORDER BY ts DESC
        ", $date_from, $date_now, $q2_terms, $q3_terms, $q4_terms);

        if ($num_of_logs > 0) {
            if ($course_id > 0) {
                $tool_content .= "<div class='col-sm-12'><div class='alert alert-info'>
                                    <i class='fa-solid fa-circle-info fa-lg'></i>
                                    <span>$langCourse: " . q(course_id_to_title($course_id)) . "</span>
                                  </div></div>";
            }
            ...
            if ($module_id > 0) {
                if ($module_id == MODULE_ID_USERS) {
                    $tool_content .= "<div class='col-sm-12'><div class='alert alert-info'>
                                        <i class='fa-solid fa-circle-info fa-lg'></i>
                                        <span>$langModule: " . $langAdminUsers . "</span>
                                      </div></div>";
                } elseif ($module_id == MODULE_ID_TOOLADMIN) {
                    $tool_content .= "<div class='col-sm-12'><div class='alert alert-info'>
                                        <i class='fa-solid fa-circle-info fa-lg'></i>
                                        <span>$langModule: " . $langExternalLinks . "</span>
                                      </div></div>";
                } elseif ($module_id == MODULE_ID_ABUSE_REPORT) {
                    $tool_content .= "<div class='col-sm-12'><div class='alert alert-info'>
                                        <i class='fa-solid fa-circle-info fa-lg'></i>
                                        <span>$langModule: " . $langAbuseReport . "</span>
                                      </div></div>";
                } elseif ($module_id == MODULE_ID_COURSEINFO) {
                    $tool_content .= "<div class='col-sm-12'><div class='alert alert-info'>
                                        <i class='fa-solid fa-circle-info fa-lg'></i>
                                        <span>$langConfig</span>
                                      </div></div>";
                } else {
                    $tool_content .= "<div class='col-sm-12'><div class='alert alert-info'>
                                        <i class='fa-solid fa-circle-info fa-lg'></i>
                                        <span>$langModule: " . $modules[$module_id]['title'] . "</span>
                                      </div></div>";
                }
            }

            $tool_content .= "<div class='col-12'><div class='table-responsive'>
                              <table id='log_results_table' class='table-default table-logs'>
                              <thead>
                                <tr>
                                    <th>$langDate</th>
                                    <th>$langUser</th>
            ...
            $tool_content .= "<th>$langAction</th><th>$langDetail</th>";
            $tool_content .= "</tr></thead><tbody style='word-wrap: break-word; word-break: break-word;'>";
            ...
            foreach ($sql as $r) {
                ...
                if (!empty($r->details) && is_string($r->details)) {
                    ...
                    if (strpos($r->details, 'a:') === 0) {

                        $tmp = @unserialize($r->details);
                ...
                // ================= DISPLAY =================
                if (is_array($details_array) && isset($details_array['title'])) {

                    $tool_content .= "<td>" . q($details_array['title']) . "</td>";
                } else {

                    $details_str = is_string($r->details) ? q($r->details) : '';

                    if ($course_id == 0 || $module_id == 0) {
                        $tool_content .= "<td>" .
                            $this->other_action_details($r->action_type, $details_str) .
                            "</td>";
                    } else {
                        $tool_content .= "<td>" .
                            $this->course_action_details($r->module_id, $details_str, $r->action_type) .
                            "</td>";
                    }
                }
    ...
    public function course_action_details($module_id, $details, $type = null)
    {
        ...
        switch ($module_id) {
            case MODULE_ID_AGENDA:
                $content = $this->agenda_action_details($details);
                break;
            case MODULE_ID_LINKS:
                $content = $this->link_action_details($details);
                break;
            case MODULE_ID_DOCS:
                $content = $this->document_action_details($details);
                break;
            case MODULE_ID_ANNOUNCE:
                $content = $this->announcement_action_details($details);
                break;
            case MODULE_ID_ASSIGN:
                $content = $this->assignment_action_details($details);
                break;
            case MODULE_ID_VIDEO:
                $content = $this->video_action_details($details);
                break;
            case MODULE_ID_MESSAGE:
                $content = $this->dropbox_action_details($details);
                break;
            case MODULE_ID_GROUPS:
                $content = $this->group_action_details($details);
                break;
            case MODULE_ID_DESCRIPTION:
                $content = $this->description_action_details($details);
                break;
            case MODULE_ID_GLOSSARY:
                $content = $this->glossary_action_details($details);
                break;
            case MODULE_ID_LP:
                $content = $this->lp_action_details($details);
                break;
            case MODULE_ID_EXERCISE:
                $content = $this->exercise_action_details($details);
                break;
            case MODULE_ID_QUESTIONNAIRE:
                $content = $this->questionnaire_action_details($details);
                break;
            case MODULE_ID_WIKI:
                $content = $this->wiki_action_details($details);
                break;
            case MODULE_ID_USERS:
                $content = $this->course_user_action_details($details, $type);
                break;
            case MODULE_ID_TOOLADMIN:
                $content = $this->external_link_action_details($details);
                break;
            case MODULE_ID_ABUSE_REPORT:
                $content = $this->abuse_report_action_details($details);
                break;
            case MODULE_ID_WALL:
                $content = $this->wall_action_details($details);
                break;
            case MODULE_ID_COURSEINFO:
                $content = $this->modify_course_action_details($details);
                break;
            case MODULE_ID_SETTINGS:
                $content = $this->modify_course_action_details($details); // <-- for backward compatibility only !!!
                break;
            case MODULE_ID_GRADEBOOK:
                $content = $this->gradebook_action_details($details);
                break;
            case MODULE_ID_ATTENDANCE:
                $content = $this->attendance_action_details($details);
                break;
            case MODULE_ID_TC:
                $content = $this->tc_action_details($details);
                break;
            case MODULE_ID_CHAT:
                $content = $this->chat_action_details($details);
                break;
            /*case MODULE_ID_MINDMAP: $content = $this->mindmap_action_details($details);
                break; */
            default:
                $content = $langUnknownModule;
                break;
        }
        return $content;
    }
    ...
    private function other_action_details($logtype, $details)
    {
        ...
        switch ($logtype) {
            case LOG_CREATE_COURSE:
                $content = $this->create_course_action_details($details);
                break;
            case LOG_DELETE_COURSE:
                $content = $this->delete_course_action_details($details);
                break;
            case LOG_PROFILE:
                $content = $this->profile_action_details($details);
                break;
            case LOG_LOGIN_FAILURE:
                $content = $this->login_failure_action_details($details);
                break;
            case LOG_DELETE_USER:
                $content = $this->delete_user_action_details($details);
                break;
            default:
                $content = $langUnknownAction;
                break;
        }
        return $content;
    }
    ...
    private function create_course_action_details($details)
    {
        ...
        $content  = "$langTitle &laquo;" . q($data['title']) . "&raquo;";
        $content .= "&nbsp;(" . q($data['code']) . ")";
    ...
    private function modify_course_action_details($details)
    {
        ...
        if (isset($details['visible'])) {
            switch ($details['visible']) {
                case COURSE_CLOSED:
                    $mes = q($langIn) . "&nbsp;&laquo;" . q($langClosedCourse) . "&raquo;";
                    break;
                case COURSE_REGISTRATION:
                    $mes = q($langIn) . "&nbsp;&laquo;" . q($langRegCourse) . "&raquo;";
                    break;
                case COURSE_OPEN:
                    $mes = q($langIn) . "&nbsp;&laquo;" . q($langOpenCourse) . "&raquo;";
                    break;
                case COURSE_INACTIVE:
                    $mes = q($langIn) . "&nbsp;&laquo;" . q($langInactiveCourse) . "&raquo;";
                    break;
                default:
                    $mes = '';
                    break;
            ...
            $lm = '';
            switch ($details['id']) {
                case SETTING_BLOG_COMMENT_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
                    $mes = "$lm $langBlogComment";
                    break;
                case SETTING_BLOG_RATING_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
                    $mes = "$lm $langBlogRatingLog";
                    break;
                case SETTING_BLOG_SHARING_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
                    $mes = "$lm $langBlogSharingLog";
                    break;
                case SETTING_COURSE_SHARING_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
                    $mes = "$lm $langsCourseSharing";
                    break;
                case SETTING_COURSE_RATING_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
                    $mes = "$lm $langsCourseRating";
                    break;
                case SETTING_COURSE_COMMENT_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
                    $mes = "$lm $langCourseComment";
                    break;
                case SETTING_COURSE_ANONYMOUS_RATING_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
                    $mes = "$lm $langsCourseAnonymousRating";
                    break;
                case SETTING_FORUM_RATING_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
                    $mes = "$lm $langForumRating";
                    break;
                case SETTING_COURSE_SOCIAL_BOOKMARKS_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
                    $mes = "$lm $langCourseSocialBookmarks";
                    break;
                case SETTING_COURSE_ABUSE_REPORT_ENABLE:
                    $lm = ($details['value']) ?  "$langActivate" : "$langDeactivate";
    ...
    private function questionnaire_action_details($details): string
    {
        ...
        if (!empty($details['legend'])) {
            switch ($details['legend']) {
                case 'submit_answers':
                    $content .= "&nbsp;&mdash;&nbsp; $langSubmit";
                    break;
                case 'purge_results':
                    $content .= "&nbsp;&mdash;&nbsp; $langPurgeExerciseResults";
                    break;
            }
        }
        return $content;
    ...
    private function course_user_action_details($details, $type): string
    {
        ...
        switch ($details['right']) {
            case '+5':
                $content = $langRegistration;
                $content .= "&nbsp;&laquo" . display_user($details['uid'], false, false) . "&raquo";
                break;
            case '-5':
                $content = $langUnCourse;
                $content .= "&nbsp;&laquo" . display_user($details['uid'], false, false) . "&raquo";
                break;
            case '0':
                $content = "$langUnCourse $langOfUserS";
                $content .= "&nbsp;&laquo" . display_user($details['uid'], false, false) . "&raquo";
                break;
            case '+1':
                $content = "$langTheU &nbsp;&laquo" . display_user($details['uid'], false, false) . "&raquo;&nbsp;";
            ...
            case '-4':
                ...
                break;
            case '+10':
                $content = "$langAddGUser&nbsp;&laquo" . display_user($details['uid'], false, false) . "&raquo&nbsp;";
                break;
        }
        return $content;
    }
    ...
    private function gradebook_action_details($details)
    {
        ...
        $content = "";
        $separator = function () use (&$content) {
    ...
    public function get_action_names($action_type)
    {
        ...
        switch ($action_type) {
            ...
            case LOG_DELETE:
                return $langDelete;
            case LOG_PROFILE:
                return $langModProfile;
            case LOG_CREATE_COURSE:
                return $langFinalize;
            case LOG_DELETE_COURSE:
                return $langCourseDel;
            case LOG_MODIFY_COURSE:
                return $langModifyInfo;
            case LOG_LOGIN_FAILURE:
                return $langLoginFailures;
            case LOG_DELETE_USER:
                return $langUnregUsers;
            default:
                return $langUnknownAction;
        }
    }
