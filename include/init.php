...
if (file_exists('config/config.php')) {
    if(get_config('mentoring_always_active')){// ANEKSARTHTH PLATFORM
    ...
    if(!get_config('mentoring_always_active')){
        ...
        if(isset($_GET['goToMentoring']) and $_GET['goToMentoring'] == 'true'){
            $_SESSION['mentoring_platform'] = 1;
        }
        if(isset($_GET['goToMentoring']) and $_GET['goToMentoring'] == 'false'){
            if(!get_config('mentoring_always_active')){
                unset($_SESSION['mentoring_platform']);
            }
            
            unset($_SESSION['program_code']);
            unset($_SESSION['program_id']);
            unset($_SESSION['mentoring_group_id']);
            unset($mentoring_platform);
    ...
    if(get_config('mentoring_platform')){
        if(isset($_SESSION['mentoring_platform']) and $_SESSION['mentoring_platform']){
            $mentoring_platform = $_SESSION['mentoring_platform'];
        }
        if(isset($_SESSION['program_code']) and $_SESSION['program_code']){
            $mentoring_program_code = $_SESSION['program_code'];
            $get_id_program = Database::get()->queryArray("SELECT id FROM mentoring_programs WHERE code = ?s",$mentoring_program_code);
            if(count($get_id_program) > 0){
                foreach($get_id_program as $i){
                    $mentoring_program_id = $i->id;
                }
                $_SESSION['program_id'] = $mentoring_program_id;
            ...
            if(isset($_SESSION['uid'])){
                $currentProgramLanguage = Database::get()->querySingle("SELECT lang FROM mentoring_programs WHERE id = ?d",$mentoring_program_id)->lang;
                $languageInterfaceProgram = $currentProgramLanguage;
                // If program language is different from global language,
                // include more messages
                if ($language != $languageInterfaceProgram) {
                    $language = $languageInterfaceProgram;
                    // include_messages
                    include "lang/$language/common.inc.php";
                    $extra_messages = "config/{$language_codes[$language]}.inc.php";
                    if (file_exists($extra_messages)) {
