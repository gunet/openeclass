    ...
    foreach ($tools_sections as $section) {
        ...
        foreach ($result as $toolsRow) {
            ...
            if ($rich and ($mid == MODULE_ID_MESSAGE or $mid == MODULE_ID_ANNOUNCE)) {
                if ($mid == MODULE_ID_MESSAGE) {
                    ...
                    if ($new_msgs != 0) {
                        array_push($sideMenuText, '<b class="pe-2">' . q($modules[$mid]['title']) .
                            " $mail_status<span class='badge rounded-pill bgLightBlue text-white TextExtraBold float-end' style='height:25px width:25px; font-size:12px;'>$new_msgs</span></b>");
                    } else {
                ...
                array_push($sideMenuText, '<b class="pe-2">' . q($modules[$mid]['title']) .
                    "<button class='badge rounded-pill bgLightBlue text-white TextExtraBold float-end border-0' style='height:25px width:25px; font-size:12px;'>$new_docs</button></b>");
            } else {
