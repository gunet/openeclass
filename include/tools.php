    ...
    foreach ($tools_sections as $section) {
        ...
        foreach ($result as $toolsRow) {
            ...
            if ($rich and ($mid == MODULE_ID_MESSAGE or $mid == MODULE_ID_ANNOUNCE)) {
                if ($mid == MODULE_ID_MESSAGE) {
                    ...
                    if ($new_msgs != 0) {
                        array_push($sideMenuText, '<b class="pe-2">' . q($modules[$mid]['title']) .
                            " $mail_status<span class='badge rounded-pill bgLightBlue text-white float-end' style='padding:5px;'>$new_msgs</span></b>");
                    } else {
                ...
                array_push($sideMenuText, '<b class="pe-2">' . q($modules[$mid]['title']) .
                    "<button class='badge rounded-pill bgLightBlue text-white float-end' style='padding:5px;'>$new_docs</button></b>");
            } else {
