...
function upgrade_course_old($code, $lang, $extramessage = '') {
    ...
    Database::get($code)->queryFunc("SELECT type FROM questions", function ($f) use ($code, $langTable, $OK, $BAD) {
        if (empty($f->type)) {
            if (Database::get($code)->query("UPDATE `questions` SET type=1")) {
    ...
    if (!mysql_table_exists($code, 'assignments')) {
        Database::get($code)->query("CREATE TABLE `assignments` (
    ...
    if (!mysql_table_exists($code, 'assignment_submit')) {
        Database::get($code)->query("CREATE TABLE `assignment_submit` (
                        `id` int(11) NOT NULL auto_increment,
    ...
    if (!mysql_table_exists($code, 'videolink')) {
        Database::get($code)->query("CREATE TABLE videolinks (
                        id int(11) NOT NULL auto_increment,
                           url varchar(200),
                           titre varchar(200),
                           description text,
    ...
    if (!mysql_table_exists($code, 'link_categories')) {
        Database::get($code)->query("CREATE TABLE `link_categories` (
    ...
    Database::get()->queryFunc("SELECT url FROM `liens` WHERE url REGEXP '\"target=_blank$'", function ($u) {
        $temp = $u->url;
        $newurl = preg_replace('#\s*"target=_blank#', '', $temp);
        Database::get()->query("UPDATE liens SET url=?s WHERE url=?s, $newurl, $temp");
    });
    ...
    if (!mysql_table_exists($code, 'dropbox_file')) {
        ...
        Database::get($code)->query("CREATE TABLE dropbox_file (
    ...
    if (mysql_table_exists($code, 'work'))
        Database::get()->query("DROP TABLE `work`");
    if (mysql_table_exists($code, 'work_student'))
        Database::get()->query("DROP TABLE `work_student`");
    ...
    $event_counter = 0;
    Database::get($code)->queryFunc($sql, function ($myAgenda) use (&$event_counter, &$lesson_agenda, $code) {
        $lesson_agenda[$event_counter]['id'] = $myAgenda->id;
        $lesson_agenda[$event_counter]['title'] = $myAgenda->titre;
        $lesson_agenda[$event_counter]['content'] = $myAgenda->contenu;
        $lesson_agenda[$event_counter]['date'] = $myAgenda->day;
        $lesson_agenda[$event_counter]['time'] = $myAgenda->hour;
        $lesson_agenda[$event_counter]['duree'] = $myAgenda->lasting;
        $lesson_agenda[$event_counter]['lesson_code'] = $code;
        $event_counter++;
    });

    for ($j = 0; $j < $event_counter; $j++) {
        Database::get()->query("INSERT INTO agenda (lesson_event_id, titre, contenu, day, hour, lasting, lesson_code)
                                VALUES (" . quote($lesson_agenda[$j]['id']) . ",
                                        ...
                                        " . quote($lesson_agenda[$j]['lesson_code']) . ")");
    }
    // end of agenda
    // Group table
    if (!mysql_table_exists($code, 'group_documents')) {
        Database::get($code)->query("CREATE TABLE `group_documents` (
    ...
    if (!mysql_table_exists($code, 'lp_module')) {
        Database::get($code)->query("CREATE TABLE `lp_module` (
    ...
    if (!mysql_table_exists($code, 'lp_learnPath')) {
        Database::get($code)->query("CREATE TABLE `lp_learnPath` (
    ...
    if (!mysql_table_exists($code, 'lp_rel_learnPath_module')) {
        Database::get($code)->query("CREATE TABLE `lp_rel_learnPath_module` (
    ...
    if (!mysql_table_exists($code, 'lp_asset')) {
        Database::get($code)->query("CREATE TABLE `lp_asset` (
    ...
    if (!mysql_table_exists($code, 'lp_user_module_progress')) {
        Database::get($code)->query("CREATE TABLE `lp_user_module_progress` (
    ...
    if (!mysql_table_exists($code, 'wiki_properties')) {
        Database::get($code)->query("CREATE TABLE `wiki_properties` (
                        `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    if (!mysql_table_exists($code, 'wiki_acls')) {
        Database::get($code)->query("CREATE TABLE `wiki_acls` (
    ...
    if (!mysql_table_exists($code, 'wiki_pages')) {
        Database::get($code)->query("CREATE TABLE `wiki_pages` (
                        `id` int(11) unsigned NOT NULL auto_increment,
    ...
    if (!mysql_table_exists($code, 'wiki_pages_content')) {
        Database::get($code)->query("CREATE TABLE `wiki_pages_content` (
                        `id` int(11) unsigned NOT NULL auto_increment,
    ...
    if (!mysql_table_exists($code, 'survey')) {
        Database::get($code)->query("CREATE TABLE `survey` (
                        ...
                        PRIMARY KEY  (`sid`)
                                ) "); //TYPE=MyISAM COMMENT='For the questionnaire module';
    }
    if (!mysql_table_exists($code, 'survey_answer')) {
        Database::get($code)->query("CREATE TABLE `survey_answer` (
    ...
    if (!mysql_table_exists($code, 'survey_answer_record')) {
        Database::get($code)->query("CREATE TABLE `survey_answer_record` (
    ...
    if (!mysql_table_exists($code, 'survey_question')) {
        Database::get($code)->query("CREATE TABLE `survey_question` (
    ...
    if (!mysql_table_exists($code, 'survey_question_answer')) {
        Database::get($code)->query("CREATE TABLE `survey_question_answer` (
    ...
    if (!mysql_table_exists($code, 'poll')) {
        Database::get($code)->query("CREATE TABLE `poll` (
                        ...
                        `active` int(11) NOT NULL default '0',
                        PRIMARY KEY  (`pid`)
                                ) "); //TYPE=MyISAM COMMENT='For the poll module';
    } else {
        Database::get($code)->query("ALTER TABLE `poll` CHANGE `creation_date` `creation_date` DATE NOT NULL DEFAULT '1000-10-10'");
        Database::get($code)->query("ALTER TABLE `poll` CHANGE `start_date` `start_date` DATE NOT NULL DEFAULT '1000-10-10'");
        Database::get($code)->query("ALTER TABLE `poll` CHANGE `end_date` `end_date` DATE NOT NULL DEFAULT '1000-10-10'");
    }

    if (!mysql_table_exists($code, 'poll_answer_record')) {
        Database::get($code)->query("CREATE TABLE `poll_answer_record` (
                        `arid` int(11) NOT NULL auto_increment,
                        `aid` bigint(12) NOT NULL default '0',
                        `question_text` varchar(250) NOT NULL default '',
                        `question_answer` varchar(250) NOT NULL default '',
    ...
    if (!mysql_table_exists($code, 'poll_question')) {
        Database::get($code)->query("CREATE TABLE `poll_question` (
    ...
    if (!mysql_table_exists($code, 'poll_question_answer')) {
        Database::get($code)->query("CREATE TABLE `poll_question_answer` (
    ...
    if (!mysql_table_exists($code, 'actions')) {
        Database::get($code)->query("CREATE TABLE actions (
                        id int(11) NOT NULL auto_increment,
    ...
    if (!mysql_table_exists($code, 'logins')) {
        Database::get($code)->query("CREATE TABLE logins (
                        id int(11) NOT NULL auto_increment,
                           user_id int(11) NOT NULL,
    ...
    if (!mysql_table_exists($code, 'actions_summary')) {
        Database::get($code)->query("CREATE TABLE actions_summary (
                        id int(11) NOT NULL auto_increment,
                           module_id int(11) NOT NULL,
    ...
    if (!mysql_table_exists($code, 'exercise_user_record')) {
        Database::get($code)->query("CREATE TABLE `exercise_user_record` (
                        ...
                        PRIMARY KEY  (`eurid`)
                                ) "); //TYPE=MyISAM COMMENT='For the exercise module';
    }
    ...
    if (!mysql_field_exists("$code", 'exercices', 'StartDate'))
        ...
        Database::get($code)->query("ALTER TABLE `exercices` CHANGE `StartDate` `StartDate` DATE NULL DEFAULT NULL");
    if (!mysql_field_exists("$code", 'exercices', 'EndDate'))
        ...
        Database::get($code)->query("ALTER TABLE `exercices` CHANGE `EndDate` `EndDate` DATE NULL DEFAULT NULL");
    ...
    Database::get()->query('UPDATE exercices
                  SET StartDate = NOW(),
    ...
    Database::get()->queryFunc("SELECT id, secretDirectory FROM student_group", function ($group) use ($code) {
    ...
    if (!mysql_field_exists("$code", 'video', 'path')) {
        ...
        Database::get($code)->queryFunc("SELECT url, id FROM video", function ($row) use ($code) {
    ...
    Database::get($code)->query("UPDATE `accueil`
                        SET `id` = `id` + 1000
    ...
    Database::get($code)->query("DELETE FROM accueil WHERE (id = 12 OR id = 13 OR id = 11 OR id=6)");
    update_field("accueil", "lien", "../../modules/agenda/agenda.php", "id", 1);
    Database::get($code)->query("UPDATE accueil SET visible = '0', admin = '1' WHERE id = 8 LIMIT 1");
    ...
    Database::get()->queryFunc("SELECT id,lien,image,address FROM accueil", function ($u) {
        $oldlink_lien = $u->lien;
        ...
        if ($oldlink_lien != $newlink_lien or
            ...
            Database::get()->query("UPDATE accueil SET
    ...
    Database::get()->queryFunc("SELECT image FROM accueil", function ($u) {
        if ($u->image == '../../../images/npage.gif') {
            update_field("accueil", "image", "external_link", "image", "../../../images/npage.gif");
        }
        if ($u->image == '../../../images/page.gif') {
            update_field("accueil", "image", "external_link", "image", "../../../images/page.gif");
        }
        if ($u->image == 'travaux.png') {
    ...
    if (mysql_table_exists($code, 'introduction')) {
        Database::get($code)->queryFunc("SELECT texte_intro FROM introduction", function ($text) use($code, $langMoveIntroText, $BAD) {
            if (Database::get()->query("UPDATE cours SET description=?s WHERE code=?s", $text->texte_intro, $code)) {
                Database::get($code)->query("DROP TABLE IF EXISTS introduction");
            } else {
    ...
    Database::get()->query("DROP TABLE IF EXISTS access");
    Database::get()->query("DROP TABLE IF EXISTS banlist");
    Database::get()->query("DROP TABLE IF EXISTS config");
    Database::get()->query("DROP TABLE IF EXISTS disallow");
    Database::get()->query("DROP TABLE IF EXISTS forum_access");
    Database::get()->query("DROP TABLE IF EXISTS forum_mods");
    Database::get()->query("DROP TABLE IF EXISTS headermetafooter");
    Database::get()->query("DROP TABLE IF EXISTS priv_msgs");
    Database::get()->query("DROP TABLE IF EXISTS ranks");
    Database::get()->query("DROP TABLE IF EXISTS sessions");
    Database::get()->query("DROP TABLE IF EXISTS themes");
    Database::get()->query("DROP TABLE IF EXISTS whosonline");
    Database::get()->query("DROP TABLE IF EXISTS words");
    // bogart: Update code for forum functionality END
    // remove tables liste_domains. Used for old statistics module
    Database::get()->query("DROP TABLE IF EXISTS liste_domaines");
...
function document_upgrade_file($path, $data) {
    ...
    $rows = Database::get()->queryFunc("SELECT * FROM $table WHERE path = ?s", function ($current_filename) use($table, $file_date, $new_path, $db_path, $path, $data) {
                if (empty($current_filename->filename) or
                    ...
                    Database::get()->query("UPDATE $table
                                        ...
                                        format = ?s
                                        WHERE path=?s ", $old_filename, $new_path, $format, $db_path);
                    rename($path, $data . $new_path);
                } else {
                    // File wasn't renamed
                    $new_path = false;
                }
            }, $db_path)->affectedRows;
    if ($rows < 1) {
        // File doesn't exist in database
        if ($table == 'document') {
            $format = quote(get_file_extension($old_filename));
            Database::get()->query("INSERT INTO document
            ...
            Database::get()->query("INSERT INTO group_documents
                                  SET path = " . quote($new_path) . ",
                                  filename = " . quote($old_filename));
        }
        rename($path, $data . $new_path);
...
function document_upgrade_dir($path, $data) {
    ...
    if ($new_path) {
        ...
        if ($data == 'document') {
            Database::get()->query("UPDATE $table SET format = '.dir'
...
function fix_document_date_and_format($code) {
    ...
    Database::get()->queryFunc("SELECT id, path FROM document WHERE date = '0000-00-00 00:00:00' OR date IS NULL", function ($file) use ($base) {
        $path = $base . $file->path;
        if (!file_exists($path)) {
            Database::get()->query("DELETE FROM document WHERE id = $file->id");
        } else {
            $file_date = quote(date('Y-m-d H:i:s', filemtime($path)));
    ...
    Database::get()->queryFunc("SELECT path, filename FROM document WHERE format='' OR format IS NULL", function ($file) use ($base) {
        $path = $base . $file->path;
        if (is_dir($path)) {
            $format = '.dir';
        } else {
        ...
        Database::get()->query("UPDATE document SET format = '$format' WHERE id = $file[id]");
    });
}
...
function fix_multiple_document_entries($table) {
    Database::get()->queryFunc("SELECT path, count(path) as c FROM $table GROUP BY path HAVING c > 1", function ($file) {
        Database::get()->query("DELETE FROM document WHERE path = %s LIMIT " . ($file->c - 1), $file->path);
    });
}
...
function upgrade_video($file, $id, $code) {
    ...
    if (rename($path_to_video . $file, $path_to_video . $safe_filename)) {
        Database::get($code)->query("UPDATE video SET path = '$safe_filename' WHERE id = '$id'");
    } else {
        echo "$langWarnVideoFile $path_to_video.$file $langGroupNone!<br>";
        Database::get($code)->query("DELETE FROM video WHERE id = '$id'");
    }
}
