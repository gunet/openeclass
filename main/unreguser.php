...
if (!isset($_GET['doit']) or $_GET['doit'] != "yes") {
	...
	if ($is_admin) {
		...
		$tool_content .=  "<br /><a href='profile.php'>$langBack</a></div>";
		draw($tool_content, 1);
		exit;
	} else {
		...
		if (mysql_num_rows($q) == 0) {
			...
			$tool_content .=  "</li>\n";
			$tool_content .=  "<li>$langNo: <a href='profile.php'>$langBack</a>";
			$tool_content .=  "</li></ul>";
			$tool_content .= "</td></tr></table>";
		} else {
			$tool_content .=  "<div class='caution'><b>$langNotice: </b> ";
			$tool_content .=  "$langExplain<br />\n";
			$tool_content .=  "<span class='right'><a href='profile.php'>$langBack</a></span></div>\n";
		}
	}  //endif is admin
} else {
	if (isset($uid)) {
		...
		if (mysql_affected_rows() > 0) {
			...
			$tool_content .=  "<p class='right'><a href='profile.php'>$langBack</a></p><br />";
                        $tool_content .=  "</div>";
		}
	}
}
