...
class Calendar_Events {
    ...
    */ 
    ...
    public static function set_calendar_settings($show_personal, $show_course, $show_seadline){
        global $uid; 
        Database::get()->querySingle("UPDATE personal_calendar_settings SET show_personal = ?b, show_course = ?b, show_deadline = ?b WHERE user_id = ?d", $show_personal, $show_course, $show_seadline, $uid);
    }
    ...
    public static function delete_event($eventid){
        global $uid;
        if(!$event = Calendar_Events::get_event($eventid)){
            return false;
        }
        ...
        Log::record(0, MODULE_ID_PERSONALCALENDAR, LOG_DELETE, array('user_id' => $uid, 'id' => $eventid,
            'title' => $event->title,
            'content' => $content));
    }
                ...
                if (($curday > 0) && ($curday <= $numberofdays[$month])) {
                    ...
                    if(array_key_exists($curday, $events)){
                        foreach($events[$curday] as $ev){
                            $thisDayItems .= Calendar_Events::month_calendar_item($ev, Calendar_Events::$calsettings->{$ev->event_type."_color"});
                        }
   ...
   public static function month_calendar_item($event, $color){
       ...
       $formatted_calendar_item = "<a href=\"".$link."\"><div class=\"{$event->event_type}\" style=\"padding:2px;border:1px solid $color; background-color:$color;\">".$event->title."</div></a>";
       $formatted_calendar_item = "<a href=\"".$link."\"><div class=\"{$event->event_type}\" style=\"padding:2px;background-color:$color;\">".$event->title."</div></a>";
   ...
   public static function icalendar(){
       ...
       $eventlist = Calendar_Events::get_user_course_events('all');
       Calendar_Events::set_calendar_settings($show_personal_bak,$show_course_bak,$show_deadline_bak);
       Calendar_Events::get_calendar_settings();
