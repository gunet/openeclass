...
tinymce.PluginManager.add("fontawesome", function(editor, url) {
    ...
    iconsData.forEach(icon => {
        icon.categories.forEach(cat => {
            const catId = cat.toLowerCase().replace(/ (.)/g, (match, group) => group.toUpperCase()).replace(/ /g, "");
            if (!categories[catId]) categories[catId] = { title: cat, icons: [] };
            categories[catId].icons.push(icon);
        });
    });
    ...
    editor.on("init", function() {
        ...
        link.rel = "stylesheet";
        link.href = url + "/css/fontawesome.min.css";
        document.head.appendChild(link);
    });

    const generateIconGrid = (filterText = "") => {
        let html = '<div class="mce-fontawesome-container" style="max-height: 400px; overflow-y: auto;">';

        for (let catId in categories) {
            const cat = categories[catId];
            const filteredIcons = cat.icons.filter(icon =>
                icon.id.includes(filterText.toLowerCase()) ||
                (icon.filter && icon.filter.some(f => f.includes(filterText.toLowerCase())))
            );

            if (filteredIcons.length > 0) {
                html += `<div style="font-weight: bold; padding: 10px 0; border-bottom: 1px solid #ccc; margin-top:10px;">${cat.title}</div>`;
                html += '<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(40px, 1fr)); gap: 5px; padding: 10px;">';
                ...
                filteredIcons.forEach(icon => {
                    ...
                    html += `
                    <div class="mce-icon-cell js-fa-icon" 
                         title="${icon.name}" 
                         data-id="${icon.id}" 
                         data-prefix="${prefix}"
                         style="cursor: pointer; text-align: center; padding: 10px; border: 1px solid #eee;"
                         onmouseover="this.style.background='#f0f0f0'" 
                         onmouseout="this.style.background='transparent'">
                        <span class="${prefix} fa-${icon.id}" style="font-family: 'Font Awesome 5 Free', 'Font Awesome 5 Brands', FontAwesome !important; font-weight: 900; font-style: normal; display: inline-block;"></span>
                    </div>`;
                });
                html += '</div>';
            }
            ...
            body: {
                ...
                items: [
                    ...
                    },
                    {
                        type: 'htmlpanel',
                        name: 'iconList',
                        html: generateIconGrid()
                    }
                ]
            },
            buttons: [
                { type: 'cancel', text: _('Close') }
            ],
            initialData: { search: '' },

            onChange: (api, details) => {
                if (details.name === 'search') {
                    const query = api.getData().search;
                    const newHtml = generateIconGrid(query);
                }
            }
        });
        ...
        setTimeout(() => {
            ...
            if (container) {
                container.addEventListener('click', (e) => {
                    const iconCell = e.target.closest('.js-fa-icon');
                    if (iconCell) {
                        const iconId = iconCell.getAttribute('data-id');
                        const prefix = iconCell.getAttribute('data-prefix') || 'fas';
                        const iconHtml = `<span class="${prefix} fa-${iconId}"></span>&nbsp;`;
    ...
    editor.ui.registry.addIcon('fa-icon', '<svg width="24" height="24" viewBox="0 0 24 24"><path d="M14.4 6L14 4H5v17h2v-7h5.6l.4 2h7V6h-5.6z" fill-rule="evenodd"/></svg>');
